...
const HomePage = () => {
  ...
  const links = [
    { img: tele, link: contact?.telegram },
    { img: fb, link: contact?.facebook },
    { img: viber, link: contact?.viber },
  ];
  ...
  return (
    <div>
      ...
      <div className="beforeLoginHome my-5 pb-3">
        ...
        {auth && (
          <>
            <button className="w-full py-2 rounded-3 homeLoginBtn text-center text-black fw-bold" onClick={logout}>
              {loader ? <SmallSpinner /> : <i className="fas fa-right-from-bracket me-2"></i>}
              ထွက်ရန်
            </button>
          </>
        )}
        ...
        <div className="mt-4 w-full text-center d-flex align-items-center justify-content-center gap-2  py-2 cursor-pointer sidebarSocial text-center rounded-3">
          <FaViber size={28} />
          <p className=" fw-semibold">{contact?.phone}</p>
        </div>
